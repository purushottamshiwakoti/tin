$(document).ready(function() {
    ...
    $(document).ready(function() {
        ...
        // $('#export-excell').on('click', function(e) {
        //     e.preventDefault();
        //     $('input[name=should_export]').val(1);
        //     ajaxDataTable.draw();
        //
        //     // console.log(ajaxDataTable.ajax.url(ajaxDataTable.ajax.params))
        //     var params = ajaxDataTable.ajax.params();
        //
        //     // var x = JSON.stringify(params, null, 4);
        //     var loadUrl = ajaxDataTable.ajax.url()+'?'+$.param(params);
        //     window.location =ajaxDataTable.ajax.url()+x;
        //
        // });

        var ajaxDataTable = $('#dt-server-processing').DataTable({
            "processing": true,
            "serverSide": true,
            "order":[[0,'desc']],
            "searching":true,
            ...
            "ajax": {
                ...
                data: function (d)
                {
                    d.from  = $('input[name=start_date]').val();
                    d.to = $('input[name=end_date]').val();
                    d.only_departure = $('input[name=only_departure]').is(':checked');
                    d.abandoned = $('input[name=abandoned]').is(':checked');
                    d.should_export = $('input[name=should_export]').val()
                }
            },
            "drawCallback": function( settings ) {
                $('input[name=should_export]').val(0);
                var params = ajaxDataTable.ajax.params();
                params.length=-1;
                // var x = JSON.stringify(params, null, 4);
                var loadUrl = ajaxDataTable.ajax.url()+'?'+$.param(params)+'&should_export=1';
                $('#export-excell').attr('href',loadUrl);
                $('#export-pdf').attr('href',loadUrl+"&ex-type=pdf")
            },
            "columns": makeColumns($(this)),
            "createdRow": function (row,data,index) {

                $(row).addClass('deleteBox');
            }
        });
    ...
    function makeColumns(current)
    {
        if(typeof editable === 'undefined'){ editable = true;}
        if(typeof popbtn === 'undefined'){ popbtn = false;}
        if(typeof deletable === 'undefined'){deletable = true;}
        if(typeof readable === 'undefined'){readable = true;}
        var columns = [];
        current.find(".header-cols th").each(function (k,v) {
            var cur = $(v);
            var col_name = cur.attr('data-col-name');
            if(col_name != '' && col_name!=undefined && col_name != 'action')
            {
                columns.push({className:cur.attr('data-col-name'),data:cur.attr('data-col-name')});
            }
            ...
            if(col_name == 'action')
            {
                columns.push({
                    ...
                    render:function(data,type,row){
                        ...
                        buttons+= popbtn?'<a type="" href="'+current_url+'/'+data.id+'/edit" class="tabledit-pop-button table_pop btn btn-primary waves-effect waves-light" alt="Last minute" style="float: none;margin: 5px;"><span class="fas fa-anchor"></span></a>':'';
                        buttons+= editable?'<a type="" href="'+current_url+'/'+data.id+'/edit" class="tabledit-edit-button table_edit btn btn-primary waves-effect waves-light" style="float: none;margin: 5px;"><span class="fas fa-pencil-alt"></span></a>':'';
                        buttons+=readable?'<a type="" href="'+current_url+'/'+data.id+'" class="tabledit-view-button table_view btn btn-primary waves-effect waves-light" style="float: none;margin: 5px;"><span class="fas fa-info-circle"></span></a>':'';
                        buttons+=deletable?'<a type="" href="'+current_url+'/'+data.id+'"  class="tabledit-delete-button ajaxDelete btn btn-danger waves-effect waves-light" style="float: none;margin: 5px;"><span class="fas fa-trash-alt"></span></a>':'';
                        buttons+='</div>';
                        return buttons;
                    },
                    searchable:false,
